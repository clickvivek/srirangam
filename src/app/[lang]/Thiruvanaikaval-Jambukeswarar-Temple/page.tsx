...
export default async function JambukeswararTemple({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const contentPath = path.join(process.cwd(), 'src', 'content', 'Thiruvanaikaval-Jambukeswarar-Temple', `${lang}.md`);
  let content = '';
  try {
    content = fs.readFileSync(contentPath, 'utf8');
  } catch (e) {
    content = fs.readFileSync(path.join(process.cwd(), 'src', 'content', 'Thiruvanaikaval-Jambukeswarar-Temple', `en.md`), 'utf8');
  }
